...
#        H,J = region.convex_image #Envolvente binarizada (mismo tamaño que bbox)
    ...
    print('conteo Inicial:', max(np.ravel(labeled)))
...
I_axes = I_axes >= 1
I_areaConv = I_areaConv >= 1
I_solid = I_solid >= 1
# plt.close('all')
fig,ax = plt.subplots(1)
#ax.imshow(I_axesRGB,cmap='gray')
ax.imshow(I_axes,cmap='gray')
ax.set_axis_off()
plt.tight_layout()
plt.title('Relación de ejes')
plt.show()

fig,ax = plt.subplots(1)
#ax.imshow(I_areaConvRGB,cmap='gray')
ax.imshow(I_areaConv,cmap='gray')
ax.set_axis_off()
plt.tight_layout()
plt.title('Área convexa')
plt.show()

fig,ax = plt.subplots(1)
#ax.imshow(I_solidRGB,cmap='gray')
ax.imshow(I_solid,cmap='gray')
ax.set_axis_off()
plt.tight_layout()
plt.title('Solidez')
plt.show()
